...
struct ParticipantContributionBuilderTests {
    ...
    @Test func contributionSummaryMultipleTargetsFirstFindCountPerTarget() async throws {
        let gameId = UUID()
        let base = Date()
        let d1 = makeDiscovery(participantId: "alice", targetId: "us-ca", gameInstanceId: gameId, discoveredAt: base)
        let d2 = makeDiscovery(participantId: "bob", targetId: "us-ca", gameInstanceId: gameId, discoveredAt: base.addingTimeInterval(5))
        let d3 = makeDiscovery(participantId: "bob", targetId: "us-ny", gameInstanceId: gameId, discoveredAt: base.addingTimeInterval(10))
        let credits = [
