...
//
//  Deterministic lifetime stats: family classification, leaver exclusion, engine parity.
//
...
struct LifetimeStatsFamilyClassificationTests {

    @Test func emptyFamilySet_isNeverFamilyOnly() {
        let roster = [TripParticipant(userId: "u1", role: .owner, joinedAt: .now)]
        #expect(!LifetimeStatsFamilyClassification.isFamilyOnlyTrip(activeParticipants: roster, familyMemberUserIds: []))
    }

    @Test func soloParticipant_inFamilyCountsAsFamilyOnly() {
        let roster = [TripParticipant(userId: "u1", role: .owner, joinedAt: .now)]
        let family: Set<String> = ["u1", "u2"]
        #expect(LifetimeStatsFamilyClassification.isFamilyOnlyTrip(activeParticipants: roster, familyMemberUserIds: family))
    }
    ...
    @Test func leaverExcludedFromActiveRoster_forFamilyCheck() {
        ...
        #expect(LifetimeStatsFamilyClassification.isFamilyOnlyTrip(activeParticipants: active, familyMemberUserIds: family))
    }
}
...
struct LifetimeStatsRecomputeEnginePureTests {
    ...
    @Test func skipsTripWhenSubjectHasLeftAt() throws {
        ...
        let input = LifetimeStatsRecomputeInput(subjectUserId: "me", familyMemberUserIds: ["me", "owner"], trips: [trip])
    ...
    @Test func countsEndedTripForSubject() throws {
        ...
        let trip = LifetimeStatsTripInput(session: session, games: [], discoveries: [])
        let input = LifetimeStatsRecomputeInput(subjectUserId: "me", familyMemberUserIds: [], trips: [trip])
        let out = try LifetimeStatsRecomputeEngine.compute(input)
    ...
    @Test func familyOnlyTripCount_whenRosterSubsetOfFamily() throws {
        ...
        let input = LifetimeStatsRecomputeInput(subjectUserId: "me", familyMemberUserIds: family, trips: [trip])
        let out = try LifetimeStatsRecomputeEngine.compute(input)
        #expect(out.totalCompletedTrips == 1)
