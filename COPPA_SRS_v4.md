...
- **(f) Build plan (models per §2 execution rules; rides Wave 2 — the record it decorates must exist first):**
  1. *Policy core* — pure `ConsentAssurancePolicy` (level lattice, required-level compare, staleness predicate incl. the FR-104 merge) + TS mirror + parity test. **Opus / high + adversarial verify.** The one piece where a wrong answer is silent unlawful collection. **STATUS 2026-08-27: lattice + parity LANDED** — both runtimes' policies exist (TS `CONSENT_ASSURANCE_LEVELS`/`REQUIRED_CONSENT_LEVEL` in `consentRequestsCore.ts`, Swift `ConsentAssurancePolicy` in `AgeGateStore.swift`) and are parity-pinned progression-catalog style: canonical `ConsentAssuranceLattice.v1.json` byte-identical in both trees (`LicensePlateApp/Resources/` + `functions/src/`), `consentAssuranceParity.test.ts` byte-compares the copies and pins the TS lattice, `ConsentAssuranceLatticeParityTests` pins the Swift enum (closed both ways via dictionary equality). The fixture is test-only — no runtime load path, so a missing file fails parity rather than falling back. The FR-104 staleness-predicate merge still rides the re-affirmation wave.
